...
async function handlePaymentCaptured(payload) {
  ...
  try {
    ...
    if (existingUserSubscription) {
      // If upgrading to a better plan, start the new subscription immediately
      if (plan.displayOrder > existingUserSubscription.planId.displayOrder) {
        // Mark old subscription as cancelled
        existingUserSubscription.status = 'cancelled';
    ...
    const newSubscription = new UserSubscription({
      ...
      payment: {
        ...
        gatewaySignature: '', // No signature in webhook
        status: 'completed',
        receiptId: transaction.gatewayData.receiptId
      }
    });
    ...
    if (existingUserSubscription) {
      newSubscription.renewalHistory.push({
        previousSubscriptionId: existingUserSubscription._id,
        renewedAt: new Date(),
        paymentId: payment.id
      });
    }
